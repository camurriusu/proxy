...
- To make proper use of the new connection, we must be able to read incoming HTTP requests. Using `net.Conn`’s `Read()` won’t work, since it deletes the read bytes from the TCP buffer. Since we read in chunks, this is problematic. For example, if we read 200 bytes but only need to parse the first 50, the next 150 bytes are lost. Therefore, we need our own buffer in RAM, specifically a `bufio.Reader`, from which we can safely read bytes from a `net.Conn` object using `Peek()`. This also improves performance, since we extract about 4KB from the TCP buffer at a time in one system call, and from those 4KB we can read byte by byte (which is what `http.ReadRequest()` does). Without it, to read the incoming byte stream byte by byte, we would need 4096 system calls instead of one.
